...
import { $gymPart } from '@/context/gymPart'
...
const PartPage = () => {
  ...
  const isInCart = cartItems.some((item) => item.partId === gymPart.id)
  const spinnerToggleCart = useStore(removeFromCartFx.pending)
  const spinnerSlider = useStore(getGymPartsFx.pending)
  ...
  const loadGymPart = async () => {
    try {
      const data = await getGymPartsFx('/guitar-parts?limit=20&offset=0')
  ...
  const toggleToCart = () =>
    toggleCartItem(user.username, gymPart.id, isInCart)
          ...
          <div className={styles.part__inner}>
            ...
            <div className={styles.part__info}>
              <span className={`${styles.part__info__price} ${darkModeClass}`}>
                {formatPrice(gymPart.price || 0)} Br
              </span>
